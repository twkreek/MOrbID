...
public class GadgetContour extends GadgetSpatialData {
	...
	void drawone(double /*Coord*/ pctthresh, DRAWPLANE plane) {
		int n;
	 	...
	 	if (squared)
			threshold = pctthresh * (sd.getSqMax()-sd.getSqMin());
		else
			threshold = pctthresh * (sd.getMax() - sd.getMin());
		//sd.Plane(plane);
		
		for (n=0; n<sd.Side0(); n++)
		{
	...
	void drawoneslice(double /*Coord*/ threshold, int n, DRAWPLANE plane,boolean square)
	{
		...
		switch (plane) {
		case planexy:
			cntour(sd.Z(n),threshold,scratch);
			break;
		case planeyz:
			cntour(sd.X(n),threshold,scratch);
			break;                                      
		case planexz:
			cntour(sd.Y(n),threshold,scratch);
		}
	...
	double A(int i, int j) { return sd.Slice(i, j) - contourValue;}
	void cntour(double /*Coord*/ ipz, double /*Coord*/ value, double /*Coord*/ r11)
	{
	contourValue = value;

	int mdim = sd.Side0(), imax = sd.Side1(), jmax = sd.Side2();
	...
	L_1:
		...
		case planexy: 
			...
			case planexz:
				ipy = sd.X(i + factor*idx - 1);
	    		ipx = sd.Z(j + factor*idy - 1);
				pv.AddPoint(new Point3D(ipy,ipz,ipx));
				break;
			case planeyz:
				ipy = sd.Z(i + factor*idx - 1);
	    		ipx = sd.Y(j + factor*idy - 1);
